...
use crate::platform::pipeline::{
    Completion, DispatcherPort, PacketKind, RxMessage, TxTarget, WorkerQueues, worker_queues,
};
use crate::protocol::{DATA_HEADER_LEN, MAX_PACKET_SIZE, Tunnel, TunnelResult};
...
pub struct Worker {
    ...
    queues: WorkerQueues,
    completion_fd: Option<Arc<OwnedFd>>,
...
impl Worker {
    ...
    pub fn spawn(
        ...
        let completion_fd = unsafe { libc::eventfd(0, libc::EFD_NONBLOCK | libc::EFD_CLOEXEC) };
        if completion_fd < 0 {
            return Err(io::Error::last_os_error());
        }
        let completion_fd = Arc::new(unsafe { OwnedFd::from_raw_fd(completion_fd) });
        port.completion_fd = Some(Arc::clone(&completion_fd));
        worker.completion_fd = Some(completion_fd);
    ...
    fn wake_dispatcher(&self) {
        let Some(fd) = &self.completion_fd else {
            return;
        };
        let value = 1u64;
        unsafe {
...
fn packet_source(packet: &[u8]) -> Option<std::net::IpAddr> {
    match packet.first().map(|byte| byte >> 4)? {
        4 if packet.len() >= 20 => Some(std::net::IpAddr::V4(std::net::Ipv4Addr::new(
            packet[12], packet[13], packet[14], packet[15],
        ))),
        6 if packet.len() >= 40 => Some(std::net::IpAddr::V6(std::net::Ipv6Addr::from(
...
mod tests {
    ...
    use crate::platform::pipeline::{PacketKind, RxMessage, TxTarget, worker_queues};
